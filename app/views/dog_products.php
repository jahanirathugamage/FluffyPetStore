...
// filter only dog products
$dogProducts = array_filter($products, function($product) {
    return strtolower($product['animalName']) === 'dog';
});
...
foreach ($dogProducts as $product) {
    if (!in_array($product['productID'], $productIDs)) {
        $uniqueDogProducts[] = $product;
        $productIDs[] = $product['productID'];
    }
}
$dogProducts = $uniqueDogProducts;

// Pagination
$perPage = 9;
$page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
$totalProducts = count($dogProducts);
$totalPages = ceil($totalProducts / $perPage);
$offset = ($page - 1) * $perPage;
$dogProductsPaginated = array_slice($dogProducts, $offset, $perPage);
?>
...
<div class="flex flex-col items-center justify-left m-[20px] sm:m-[40px] gap-[10px] font-[Montserrat]">
    <div class="w-full md:w-[870px] flex justify-between items-center md:gap-[60px] mb-2">
        <h3 class="font-bold text-[24px] md:text-[30px]"><?= htmlspecialchars($dogProducts[0]['animalName'] ?? 'Dog') ?> Products</h3>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 gap-[20px] md:gap-[60px] w-full max-w-[870px]">
        <?php if (!empty($dogProductsPaginated)): ?>
            <?php foreach ($dogProductsPaginated as $product): ?>
    ...
    document.querySelectorAll('.add-to-fav-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if(!exists){
                ...
                // Optional: call renderFavorites() if you have it defined elsewhere
