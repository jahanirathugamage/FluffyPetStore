...
// filter only rabbit products
$rabbitProducts = array_filter($products, function($product) {
    return strtolower($product['animalName']) === 'rabbit';
});

// keep only the first instance of each product (by productID)
$uniqueRabbitProducts = [];
$productIDs = [];
foreach ($rabbitProducts as $product) {
    if (!in_array($product['productID'], $productIDs)) {
        $uniqueRabbitProducts[] = $product;
        $productIDs[] = $product['productID'];
    }
}
$rabbitProducts = $uniqueRabbitProducts;

// Pagination
$perPage = 9;
$page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
$totalProducts = count($rabbitProducts);
$totalPages = ceil($totalProducts / $perPage);
$offset = ($page - 1) * $perPage;
$rabbitProductsPaginated = array_slice($rabbitProducts, $offset, $perPage);
?>
...
<div class="flex flex-col items-center justify-left m-[20px] sm:m-[40px] gap-[10px] font-[Montserrat]">
    ...
    <div class="w-full md:w-[870px] flex justify-between items-center md:gap-[60px] mb-2">
        <h3 class="font-bold text-[24px] md:text-[30px]"><?= htmlspecialchars($rabbitProducts[0]['animalName'] ?? 'Rabbit') ?> Products</h3>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 gap-[20px] md:gap-[60px] w-full max-w-[870px]">
        <?php if (!empty($rabbitProductsPaginated)): ?>
            <?php foreach ($rabbitProductsPaginated as $product): ?>
                <div class="w-full md:w-[250px] h-auto border-[2px] md:border-[3px] border-black flex flex-col items-center justify-center gap-[6px] p-2">
                    ...
                    <p class="font-regular text-[#5A5A5A] text-[12px] md:text-[14px]"><?= strtoupper(htmlspecialchars($product['categoryName'])) ?></p>
                    <a href="/FluffyPetStore/app/views/product_detail.php?productID=<?= $product['productID'] ?>&specID=<?= $product['specificationID'] ?>" class="h-[48px] md:h-[56px] flex items-center justify-center text-center">
                        <p class="font-medium text-black text-[16px] md:text-[20px] text-center hover:underline"><?= htmlspecialchars($product['productName']) ?></p>
                    </a>
                    <p class="font-regular text-black text-[16px] md:text-[20px] text-center">LKR <?= number_format($product['productPrice'], 2) ?></p>
