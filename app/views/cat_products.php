...
$catProducts = array_filter($products, function($product) {
    return strtolower($product['animalName']) === 'cat';
});
...
$totalProducts = count($catProducts);
$totalPages = ceil($totalProducts / $perPage);
$offset = ($page - 1) * $perPage;
$catProductsPaginated = array_slice($catProducts, $offset, $perPage);
?>
...
<div class="flex flex-col items-center justify-left m-[20px] sm:m-[40px] gap-[10px] font-[Montserrat]">
    ...
    <div class="w-full md:w-[870px] flex justify-between items-center md:gap-[60px] mb-2">
        <h3 class="font-bold text-[24px] md:text-[30px]"><?= htmlspecialchars($product['animalName']) ?> Products</h3>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 gap-[20px] md:gap-[60px] w-full max-w-[870px]">
        <?php if (!empty($catProductsPaginated)): ?>
            <?php foreach ($catProductsPaginated as $product): ?>
                <!-- Product Item -->
